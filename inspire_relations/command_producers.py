...
def produce_element_block(labels=None, properties=None, variable_name=None):
    if not labels: labels = []
    if not properties: properties = {}
    if not variable_name: variable_name = ''
...
def produce_node_block(labels=None, properties=None, variable_name=None):

    return '({})'.format(produce_element_block(labels, properties,
                                               variable_name))


def produce_relation_block(relation_type, properties=None, variable_name=None,
...
def create_node_from_csv(file_name, labels, properties):
...
def create_index(label, property_name):
    ...
    return 'CREATE INDEX ON :{label}({property_name})'.format(
        label = label,
